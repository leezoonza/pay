...
public class RedisVerificationRepository implements VerificationRepository {
    ...
    public void save(Verification verification) {
        ...
        Duration timeToLive = Duration
                .between(Instant.now(), verification.getExpiresAt())
                .plus(EXPIRATION_GRACE_PERIOD);

        redisTemplate.opsForValue().set(KEY_PREFIX + verification.getId(), value, timeToLive);
    }

    @Override
    public Optional<Verification> findById(String id) {
        return Optional.ofNullable(redisTemplate.opsForValue().get(KEY_PREFIX + id))
                .map(value -> objectMapper.readValue(value, Verification.class));
    }
}
